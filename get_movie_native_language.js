...
function getMovieName(ffProbeData) {
  // 1. Try format tags
  if (
    ffProbeData.format &&
...
async function getMovieLanguage(movieName, tmdbToken) {
  console.log("Fetching movie language for:", movieName);

  const getMovieUrl = new URL("https://api.themoviedb.org/3/search/movie");
  getMovieUrl.searchParams.append("query", movieName);
  const movieData = await fetch(getMovieUrl, {
    headers: {
  ...
  const language = movieJson.results[0]?.original_language || null;

  console.log("Native language for movie returned: ", language);
...
module.exports = async (args) => {
  ...
  const movieName = getMovieName(ffProbeData);
  if (!movieName) {
    console.error("No movie name found in ffProbeData");
    return {
  ...
  const language = await getMovieLanguage(movieName, token);
